...
describe("FormMessage component", () => {
  ...
  it("should display a content.", () => {
    const { queryByText } = render(
      <FormMessage content="Test message" visible />
    );
  ...
  it("should display an error type message.", () => {
    const { container } = render(
      <FormMessage error content="test message" visible />
    );

    const iconElement = container.querySelector(
      'span[data-src="/images/icon-alert-circle.svg"]'
    );
  ...
  it("should display a warning type message.", () => {
    const { container } = render(
      <FormMessage warning content="test message" visible />
    );
  ...
  it("should display a success type message.", () => {
    const { container } = render(
      <FormMessage success content="test message" visible />
    );
  ...
  it("should display an info type message.", () => {
    const { container } = render(
      <FormMessage info content="test message" visible />
    );

    const iconElement = container.querySelector(
      'span[data-src="/images/icon-info.svg"]'
    );
  ...
  it("should hide after timeout.", () => {
    const { queryByText } = render(
      <FormMessage content="test message" visible timeout={1000} />
    );
