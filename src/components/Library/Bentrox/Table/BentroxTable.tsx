...
export interface BentroxTableProps {
  headers?: {
    label?: string;
    style?: string;
    key?: string;
...
const BentroxTable = ({
  ...
  const styleTable = useMemo(
    () =>
      css(
        styleTableDefault,
        loading === BentroxLoading.pagination && styleTableLoading,
        loading === BentroxLoading.data && styleTableDataLoading,
  ...
  const isActionItemActive = useCallback(
    (id: string | number) =>
      loading === BentroxLoading.actions &&
      (id === activeActionRow ||
  ...
  const handleCreateKey = useCallback(
    (item: any, i: number) => {
      ...
      key = item[0]?.id ? item[0]?.id : key; // TO-DO: remove the data from BentroxTable usage
  ...
  const renderRows = useCallback(
    (item: any, i: any, isParent = true) => {
      ...
      return (
        <Draggable
          key={`Bentrox-table-${fragmentKey}`}
          draggableId={`Bentrox-table-${fragmentKey}`}
          index={i}
                        ...
                        {() => (
                          <>
                            <Dropdown
                              trigger={
                                <SvgIcon
                                  path="fa/solid/ellipsis-vertical"
                              ...
                              {() => (
                                <SortableTableHeader
                                  ...
                                  isLoading={
                                    ...
                                    loading === BentroxLoading.sorting
                                  }
