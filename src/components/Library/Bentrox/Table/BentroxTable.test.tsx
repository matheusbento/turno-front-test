...
import { loading } from "@constants/BentroxTableConstants";
import { fireEvent, render } from "@testing-library/react";

import BentroxTable from "./BentroxTable";

const mockedHeaderAction1 = vi.fn();
const mockedHeaderAction2 = vi.fn();
const mockedAction = vi.fn();
const mockedDataSortHandler = vi.fn();
const mockedOnRowClick = vi.fn();
const mockedBulkActionsHandler = vi.fn();
const mockedBulkChildrenActionsHandler = vi.fn();
...
describe("BentroxTable component", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("should display headers and rows.", () => {
    const { queryByText } = render(
      <BentroxTable headers={headers} rows={rows} disabled />
    );
  ...
  it("should display a custom styled header.", () => {
    const { container } = render(
      <BentroxTable headers={headers} rows={rows} />
    );
  ...
  it("should display and enable row's drag and drop.", () => {
    const { container } = render(
      <BentroxTable headers={headers} rows={rows} enableDragAndDrop />
    );
  ...
  it("should be able to display actions by item action key.", () => {
    const { queryByText, queryAllByText, queryAllByRole } = render(
      <BentroxTable headers={headers} rows={rows} actions={actionsObj} />
    );
